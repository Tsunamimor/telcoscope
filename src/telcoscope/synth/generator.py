...
def generate(
    *,
    ...
    plan = default_anomaly_plan(num_cells=num_cells, days=num_days, start=start, seed=seed)
    ...
    *,
    ...
    for cell in cell_rows:
        ...
        for h_idx, ts in enumerate(timestamps):
            ...
            rrc_succ = int(rrc_att * rrc_success_rate * rng.normal(1.0, archetype.noise_stddev / 4))
            rrc_succ = min(rrc_succ, rrc_att)

            # E-RAB attempts roughly track RRC (each RRC → ~1.05 E-RABs)
            erab_att_init = int(rrc_att * 1.05 * rng.normal(1.0, archetype.noise_stddev / 4))
            erab_succ_init = int(
                erab_att_init * rrc_success_rate * rng.normal(1.0, archetype.noise_stddev / 4)
            )
            ...
            erab_rel_abnormal = int(
                erab_total_rel * erab_drop * rng.normal(1.0, archetype.noise_stddev)
            )
            ...
            pdcp_vol = (
                archetype.base_dl_throughput_kbps
                * archetype.peak_users
                * 3600
                * traffic_multiplier
                * rng.normal(1.0, archetype.noise_stddev)
...
KPI_TO_COUNTERS: dict[str, list[tuple[str, float]]] = {
    ...
    "erab_drop_rate": [("ERAB_REL_ABNORMAL_ENB", +1.0), ("ERAB_REL_NORMAL_ENB", -0.5)],
